                        ...
                        <View style={styles.productOrder}>
                            <View style={styles.infoOrder}>
                                ...
                                <View style={styles.info}>
                                    ...
                                    <Text style={styles.textInfo}>Giá: {FormatPrice(e.priceProduct)}</Text>
                                    <Text style={styles.textInfo}>Số lượng: {e.sum}</Text>
                                    
                                    <Text style={[styles.textInfo, styles.textGreen]}>Ngày đặt: {dd + '/' + mm + '/' + yyyy}{'  '}
                                            {item.createdAt.toLocaleTimeString('vi_VN')}
                                    </Text>
                                </View>
                            </View>
                            <View style={styles.boxtext}>
                                <Text style={[styles.textInfo, styles.textRed]}>Tổng cộng: {FormatPrice(e.total)}</Text>
                            </View>
                            {
                                    ...
                                    [
                                        ...
                                        },
                                        {
                                            ...
                                            onPress:() =>{
                                                ...
                                                var query = postsRef.where('cartID', '==', item.cartID).get()
                                                .then(snapshot => {
                                                    snapshot.forEach(doc => {
                                                        var deleteDoc = db.collection('order').doc(doc.id).delete();
                                                    });
                                                })
                                                .catch(err => {
